...
def run_name(
    ...
    return (
        f"r5_freeze_{main_network}_{freeze_mode}_s{seed}_"
        f"from{SOURCE_STEP}_to{TARGET_STEP}_{commit[:7]}"
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Continue a matched checkpoint for a 55-step freeze ablation."
    )
    parser.add_argument("--main", choices=["t26", "k1t1"], required=True)
    parser.add_argument(
        "--freeze-mode",
    ...
    parser.add_argument("--archive-root", type=Path, default=ARCHIVE_PREFIX / "freeze")
...
def main() -> None:
    ...
    source_checkpoint = source_run_dir / f"checkpoint_step_{SOURCE_STEP:06d}.pt"
    source_model_config = source_run_dir / "model_config.json"
    for path in (source_checkpoint, source_model_config):
        if not path.is_file():
    ...
    name = run_name(args.main, args.freeze_mode, args.seed, commit)
    run_dir = args.work_root / "freeze" / name
    ...
    checkpoint = run_dir / f"checkpoint_step_{TARGET_STEP:06d}.pt"
